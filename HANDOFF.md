...
### Exploration Pass — 작은 맵 + 동적 선택지 + 몬스터 시야 이벤트
...
- 몬스터 이벤트 3종 모두 구현:
  - `sight`: 직선 끝에 회수자가 보임 — 위험 상승/로그, 그쪽 출구는 `회수자가 보인다`로 표시, 전진 시 위험 추가
  - `cross`: 갈림길을 지나감 — `기다리기`로 흘려보내거나 다른 길; 지나가는 중 이동하면 들켜 추격 시작
  - `ambush`: 움직이면 들킴 — 다음 이동 시 위험 급증·추격; `기다리기`로 완화
